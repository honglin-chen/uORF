...
class ComponentVAE(nn.Module):
    ...
    def __init__(self, input_nc, z_dim=16, full_res=False):
        ...
        self._bg_logvar = nn.Parameter(2 * torch.tensor(0.09).log(), requires_grad=False)

        self._fg_logvar = nn.Parameter(2 * torch.tensor(0.11).log(), requires_grad=False)
...
class Attention(nn.Module):
    ...
    def __init__(self, input_nc, output_nc, ngf=64, full_res=False):
        ...
        self.mlp = nn.Sequential(
            # nn.Linear((8 * 8 * ngf * 8) if full_res else (4 * 4 * ngf * 8), 128),
            nn.Linear(4 * 4 * ngf * 8, 128),
            nn.ReLU(),
            nn.Linear(128, 128),
            nn.ReLU(),
            # nn.Linear(128, (8 * 8 * ngf * 8) if full_res else (4 * 4 * ngf * 8)),
